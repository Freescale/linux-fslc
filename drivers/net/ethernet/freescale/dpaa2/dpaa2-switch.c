				      ...
				      bool linking)
{
	...
	/* If we leave an upper device, be it a bond or a bridge, find an
	 * unused FDB and use that.
	 */
	if (!linking) {
		/* A port leaving a bridge can remain under a bond. Reuse the
		 * FDB of a peer from the same LAG which already left the bridge.
		 */
		...
		/* Do not release a shared FDB while another switch port still
		 * uses it. If this is its last user, it can become standalone.
		 */
		for (i = 0; i < ethsw->sw_attr.num_ifs; i++) {
